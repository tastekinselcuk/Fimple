...
{
    public class DetailedExistingAtmMoneyQueryHandler : IRequestHandler<DetailedExistingAtmMoneyQuery, ServiceResponse<List<ExistingMoneyDto>>>
    {
                private readonly IAtmRepository _atmRepository;
        private readonly IMapper _mapper;

        public DetailedExistingAtmMoneyQueryHandler(IAtmRepository atmRepository, IMapper mapper)
        {
            _atmRepository = atmRepository;
            _mapper = mapper;
        }

        public async Task<ServiceResponse<List<ExistingMoneyDto>>> Handle(DetailedExistingAtmMoneyQuery request, CancellationToken cancellationToken)
        {
            ...
            // Tüm para birimlerini tek bir listede toplama
            var currencyDenominations = atm.Cassettes
                .SelectMany(c => c.CurrencyDenominations
                    .Where(cd => cd.Quantity > 0))
                .GroupBy(cd => new { cd.DenominationType, cd.CurrencyType }) // Burada grupluyoruz
                .Select(g => new ExistingMoneyDto
                {
                    DenominationType = g.Key.DenominationType.ToString(),
                    CurrencyType = g.Key.CurrencyType.ToString(),
                    Quantity = g.Sum(cd => cd.Quantity), // Toplam adet
